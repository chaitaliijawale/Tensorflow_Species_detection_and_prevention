...
'''
    return redirect(url_for('save_blueprint.fetch_details_for',animal_name=animal_name, file_path=filepath))


@save_blueprint.route('/<animal_name>/<file_path>', methods=['GET'])

def fetch_details_for(animal_name,file_path):
    animal_info = get_all_details_for(animal_name)
    return render_template("index.html", name=animal_name, users=user_details(), loaded=True, info=animal_info, file_path = file_path)
    '''